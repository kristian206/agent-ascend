...
const UserBanner = memo(({ 
  ...
  const getActionButtons = useCallback(() => {
    ...
    if (isOwnProfile) {
      ...
      buttons.push({ 
        label: 'Profile', 
        action: 'profile', 
        className: 'glass hover:bg-gray-800',
        icon: '👤'
      })
  ...
  return (
    ...
    >
      ...
      {BACKGROUND_OPTIONS[bannerSettings.background]?.type === 'image' ? (
        <div 
          className="absolute inset-0 bg-cover bg-center opacity-90"
          style={{ backgroundImage: `url(${BACKGROUND_OPTIONS[bannerSettings.background].value})` }}
        />
      ) : (
        <div className={`
          absolute inset-0 bg-gradient-to-br 
          ${BACKGROUND_OPTIONS[bannerSettings.background]?.value || BACKGROUND_OPTIONS.default.value}
          opacity-90
        `} />
      )}

      {/* Glass overlay */}
      <div className="absolute inset-0 bg-gray-800 bg-gray-900" />
