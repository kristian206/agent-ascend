...
export function HelpIcon({ section, className = '' }) {
  ...
  return (
    <>
      <button
        onClick={handleClick}
        className={`inline-flex items-center justify-center w-6 h-6 rounded-full bg-gray-750 hover:bg-gray-700 border border-gray-600 text-white/70 hover:text-white transition-all ${className}`}
        title="Get help"
      >
...
export function HelpPanel({ section, onClose }) {
  ...
  return createPortal(
    <>
      {/* Backdrop */}
      <div 
        className="fixed inset-0 bg-gray-8500 bg-gray-900 z-40 animate-fade-in"
        onClick={onClose}
      />
      
      {/* Panel */}
      <div className="help-panel-container fixed top-0 right-0 bottom-0 w-96 bg-gray-800 border border-gray-700 border-l border-gray-600 z-50 animate-slide-in-right overflow-y-auto">
        {/* Header */}
        <div className="sticky top-0 bg-gradient-to-r from-white/10 to-white/5 bg-gray-900 border-b border-gray-600 p-6 z-10">
          <div className="flex items-start justify-between">
            <div>
              <h2 className="text-xl font-semibold text-white">{content.title}</h2>
              <p className="text-sm text-gray-300 mt-1">Quick Help</p>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="space-y-3">
              {content.tips.map((tip, index) => (
                <div key={index} className="flex gap-3">
                  ...
                  <p className="text-sm text-gray-200 flex-1">{tip}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <a 
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-750 hover:bg-gray-700 border border-gray-600 text-gray-200 hover:text-white text-sm font-medium transition-all"
            >
          ...
          {/* Related Topics */}
          <div>
            <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">RELATED TOPICS</h3>
            <div className="space-y-2">
              <button className="w-full text-left px-3 py-2 rounded-lg hover:bg-gray-750 text-sm text-gray-200 transition-colors">
        ...
        {/* Footer */}
        <div className="sticky bottom-0 bg-black/90 bg-gray-900 border-t border-gray-700 p-6">
          ...
          <div className="flex items-center gap-2">
            {currentMark > 0 && (
              <button
                ...
                className="px-4 py-2 rounded-lg glass hover:bg-gray-800 type-list-body text-ink-700"
              >
                Back
              </button>
            )}
