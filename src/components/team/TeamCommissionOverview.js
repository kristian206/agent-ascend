  ...
  const loadTeamData = useCallback(async () => {
    ...
    try {
      ...
      if (error.code === 'failed-precondition' && error.message?.includes('index')) {
        // Firestore index is being created. Team data will be available soon.
      }
  ...
  if (isLoading) {
    return (
      <div className="glass radius-xl p-8 text-center">
        ...
        <p className="type-list-body text-gray-300">Loading team data...</p>
      </div>
    )
  }
    ...
    <div className="glass radius-xl p-6 elev-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="glass radius-lg p-4 border border-ink-100">
          <div className="type-list-label text-gray-300 mb-1">Total Commission</div>
          <div className="type-dashboard-metric text-success">${teamTotals.totalCommission}</div>
        </div>
        <div className="glass radius-lg p-4 border border-ink-100">
          <div className="type-list-label text-gray-300 mb-1">Total Revenue</div>
          <div className="type-dashboard-metric text-brand-600">${teamTotals.totalRevenue}</div>
        </div>
        <div className="glass radius-lg p-4 border border-ink-100">
          <div className="type-list-label text-gray-300 mb-1">Total Sales</div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="border-b border-ink-200">
            <tr>
              <th className="text-left py-3 px-4 type-list-label text-gray-300">Team Member</th>
              <th className="text-left py-3 px-4 type-list-label text-gray-300">Products Sold</th>
              <th className="text-right py-3 px-4 type-list-label text-gray-300">Commission</th>
              <th className="text-right py-3 px-4 type-list-label text-gray-300">Revenue</th>
              <th className="text-center py-3 px-4 type-list-label text-gray-300">Status</th>
            </tr>
          </thead>
          <tbody>
                  ...
                  <td className="py-4 px-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="type-detail-caption text-gray-300">Level {member.level || 1}</div>
                      </div>
                    </div>
                  </td>
                  <td className="py-4 px-4">
      ...
      {teamMembers.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="type-list-body text-gray-300">No team members found</p>
