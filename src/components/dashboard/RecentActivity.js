...
export default function RecentActivity() {
  ...
  return (
    <div className="bg-gray-800 bg-gray-900 p-6 rounded-2xl border border-gray-700">
      ...
      {activities.length === 0 ? (
        <p className="text-gray-300">No recent activity yet. Be the first!</p>
      ) : (
        <div className="space-y-3">
          {activities.map((activity) => (
            <div key={activity.id} className="p-3 rounded-lg bg-gray-800 flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-300">{activity.userName}</p>
                </div>
              </div>
