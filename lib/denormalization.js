...
export async function updateMonthlyTotals(saleData) {
  ...
  if (!userId || !month) {
    console.warn('Missing required fields for monthly totals update')
    return
  }
...
export async function updateUserStats(userId, points, activityType) {
  ...
  try {
    ...
    if (!userDoc.exists()) {
      console.warn('User document not found for stats update')
      return
    }
...
export async function batchUpdateDenormalizedData(updates) {
  ...
  try {
    ...
    console.log('Batch denormalization update completed')
  } catch (error) {
